...
/* Removed node:crypto usage; delegate Node work to googleHttpActions instead */
...
function buildGoogleAuthUrl(userId: string) {
  ...
  // Switch to Base64-encoded JSON state (URL-safe via URLSearchParams)
  const stateObj = { userId, nonce, ts: Date.now() };
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    ...
    try {
      ...
      return new Response(null, { status: 302, headers: { Location: "/dashboard?integration=google_connected" } });
    } catch (e: any) {
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    ...
    if (!userId) return new Response("Missing userId", { status: 400 });
    try {
      const data = await ctx.runAction(internal.googleHttpActions.listCourses, {
      ...
      return new Response(`List courses error: ${e?.message ?? "unknown"}`, {
        status: 500,
      });
    }
  }),
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    ...
    try {
      ...
      return new Response(`Calendar error: ${e?.message ?? "unknown"}`, {
        status: 500,
      });
    }
  }),
